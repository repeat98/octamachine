...
- [x] Profile nine assignment/eight-hit scenarios and nine trigger/encoder scenarios spanning all eight core engine families with aggregate ColdFire and descriptor-handler-range counts; raw outputs remain private.
...
- [ ] Map startup register operands/effects and check adaptation mechanisms against a reviewed target memory map and privilege rules; extend runtime coverage beyond these scenarios.
...
3. [WP-06 — ColdFire compatibility](work_packets/WP-06-coldfire-compatibility.md): after WP-07's map is accepted, map startup register operands/effects and consider bounded per-engine coverage beyond the nine sampled IDs.
