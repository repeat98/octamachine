...
[WP-06 — ColdFire compatibility](work_packets/WP-06-coldfire-compatibility.md) is in review in draft [PR #12](https://github.com/repeat98/octamachine/pull/12) on `work/wp-06-coldfire-compatibility`; GitHub Actions Checks run 59 passed for head `3dcb1586f918e86b8b168aeb0deeab8c6ed53a48`. The [report](reports/WP-06-coldfire.md) and firmware-free probes compare selected MCF5206E/CFV4e arithmetic, exception, privilege, level-4 interrupt-mask, VBR, EUSP, startup MOVEC, alignment, and byte-order behavior. Sanitized Gearmulator summaries cover a 1,000,000-instruction startup prefix, assignment/eight-hit runs for all 50 core synthesis IDs plus INP/MID/CTR, and nine trigger/encoder traces spanning all eight source engine families. The source profiles do not compare firmware instructions with the target CPU or attribute aggregate counts to all 44 distinct handlers. Manual comparison shows the source and target RAMBAR encodings differ; target ACR/CACR facilities exist despite QEMU model gaps. A static audit of the pinned QEMU source records CAS/CAS2 model gaps, incomplete cache-control paths, and TCG translated-code invalidation; this was not executed evidence. Reset-vector fetch, external edge-sensitive level 7, startup operand mapping, opcode/handler attribution, and physical behavior remain open. The PR body remains stale because GitHub integration writes return 403; the fetched PR head is 3dcb1586f918e86b8b168aeb0deeab8c6ed53a48, and subsequent local source-audit work is not on the remote branch.
...
- [x] Profile all 50 core synthesis IDs plus INP/MID/CTR assignment scenarios and nine trigger/encoder scenarios spanning all eight source engine families with aggregate ColdFire and descriptor-handler-range counts; raw outputs remain private.
...
- [ ] Establish atomic/cache/self-modifying-code behavior and physical/device-memory alignment where evidence is available.
...
WP-04/WP-05 and WP-35's import/c10 follow-up are accepted. WP-07's evidence is in draft PR #13; WP-08 is pushed with its packet checklist complete but needs a PR through the compare flow. WP-06 has limited executed coverage in Gearmulator plus a pinned-QEMU source audit: the tested model feature tables omit CAS/CAS2, cache-control modeling is incomplete, and TCG maintains translated-code invalidation. The source audit is not execution evidence; startup adaptation still needs operand/effect mapping after WP-07 acceptance. The local WSL checkout now has the pinned octemu source and m68k-elf-gcc, but lacks QEMU build prerequisites and local firmware/Gearmulator inputs for additional probes. WP-09 must wait for WP-08 accepted evidence. WP-35 remains Gearmulator-only prior evidence and does not replace those packet checks. The available integration can read but not merge or create pull requests; PRs #12 and #13 remain drafts and WP-08's branch remains without a PR. The local credential also cannot push to origin (403 permission denied to Aquitronic); the WP-06 update is local-only.
...
3. [WP-06 — ColdFire compatibility](work_packets/WP-06-coldfire-compatibility.md): after WP-07's map is accepted, map startup register operands/effects; continue with bounded atomic/cache/code-coherence probes when the pinned QEMU build prerequisites are available; establish per-handler attribution only if source instrumentation can do so without retaining firmware-derived addresses.
