...
[WP-07 — Memory, MMIO, and clock](work_packets/WP-07-memory-and-mmio.md) is in review on `work/wp-07-memory-and-mmio`; a draft PR is being prepared. The [report](reports/WP-07-memory-mmio.md) reconciles the Gearmulator HI08 windows, bounds SRAM and clock discrepancies, inventories peripherals through cold/cached idle, and proposes target aliases/MMIO ownership. The alias plan has not been implemented; physical board decode and clock remain unknown.
