...
3. [WP-06 — ColdFire compatibility](work_packets/WP-06-coldfire-compatibility.md): after WP-07's map is accepted, map startup register operands/effects and establish per-handler call attribution if the source instrumentation can do so without retaining firmware-derived addresses.
