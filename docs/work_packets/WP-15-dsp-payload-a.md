...
- Evidence: No packet acceptance evidence yet. Existing research is a starting point, not proof of this packet's result. Prior evidence from the octamad Machinedrum excursion: `scripts/md_reference/md_replay.cpp` runs the voice DSP's original loop outside the MD. It replays the host stream and is bit-identical to the reference in six captures covering all 50 core engines, also after relocation. Whichever payload WP-08 names the voice DSP can start from it ([WP-35 report](../reports/WP-35-octamad-md-import.md)). It is Gearmulator-only, predates the WP-03 contract, and checks no item here.
