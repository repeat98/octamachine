#!/usr/bin/env python3
"""Unpack and inventory MD OS 1.63 from the user's own update, read-only.

Imported from octamad's Machinedrum excursion (branch machinedrum-phase0,
aad3e11) by WP-35; the recipe and its pinned hashes are unchanged.
...
Layers, outermost first (docs/reports/WP-35-octamad-md-import.md,
"Firmware container and DSP load images"):
...
Usage:
  python3 scripts/md_reference/md_extract.py [--syx PATH] [--wav] [--disasm]
...
# The reference emulator this plan compares against (vendor/, fetched by
# make refs). Its loader accepts only a full 8 MiB flash dump.
...
def unpack(syx):
    ...
    if not EFT.exists():
        sys.exit(f"{EFT} missing: see scripts/md_reference/README.md")
...
def disasm(name, recs, tag):
    ...
    if not DIS.exists():
        sys.exit(f"{DIS} missing: see scripts/md_reference/README.md")
    runs = []
    for sp, ad, words in recs:
        if sp != 0:
            continue
